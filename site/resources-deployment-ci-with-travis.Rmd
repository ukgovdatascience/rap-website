...
Tell Travis what scripts to run on each commit. To allow them to run they need to be made executable with the command `chmod +x`. The scripts can do anything.  In this exampl the scripts tell Travis to build a website using rmarkdown and push it to the `gh-pages` branch on github.  GitHub will then publish the website.
